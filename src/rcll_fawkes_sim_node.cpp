...
class RcllFawkesSimNode : public fawkes::BlackBoardInterfaceListener
{
public:
	RcllFawkesSimNode(ros::NodeHandle rn, std::shared_ptr<fawkes::BlackBoard> bb)
		: BlackBoardInterfaceListener("RcllFawkesSimNode"),
		  rosnode(rn), blackboard_(bb)
	{}
	...
	finalize()
	{
		...
		blackboard_->close(ifc_zone_);
		blackboard_->close(ifc_tag_vision_);
		for (const auto &i : ifcs_tag_pos_) {
			blackboard_->close(i);
		}
		blackboard_->close(ifc_machine_signal_);
		blackboard_->close(ifc_navgraph_gen_);
		blackboard_->close(ifc_pose_);

		pub_expl_zone_info_.shutdown();
		pub_mps_marker_array_.shutdown();
		pub_mps_light_state_.shutdown();
		pub_pose_.shutdown();
		srv_navgraph_gen_.shutdown();
	}
